...
p <- dygraph(mu,main=paste("Italy: prediction (confidence ",100*confidence,"%)",sep = ""),
             xlab="Day",height=400,width=800) %>%  dySeries(c("mu.lower", "counts", "mu.upper"),label="counts")
...
p <- dygraph(signal,main=paste("Italy: signal (confidence ",100*confidence,"%)",sep = ""),
             xlab="Day",height=400,width=800) %>%  dySeries(c("lower", "med", "upper"))%>% dyLegend(show = "always", hideOnMouseOut = FALSE) 
...
counts[days.ahead]<-mu.med[days.ahead]
mu<-xts(x = as.matrix(cbind(counts,mu.lower,mu.upper)) , order.by = c(days,days.ahead))
p <- dygraph(mu,main=paste(mycounty,": total case prediction (confidence ",100*confidence,"%)",sep = ""),xlab="Day",height=400,width=800) %>%  dySeries(c("mu.lower", "counts", "mu.upper"),label="counts")
p<-p %>% dyLegend(show = "always", hideOnMouseOut = FALSE) %>%  dyShading(from = days.ahead[1], to = days.ahead[step.ahead], color = "#CCEBD6")%>% dyEvent(days.ahead[1], "Prediction", labelLoc = "bottom")
p
```
...
counts[days.ahead]<-mu.med[days.ahead]
mu<-xts(x = as.matrix(cbind(counts,mu.lower,mu.upper)) , order.by = c(days,days.ahead))
p <- dygraph(mu,main=paste(mycounty,": new case prediction (confidence ",100*confidence,"%)",sep = ""),xlab="Day",height=400,width=800) %>%  dySeries(c("mu.lower", "counts", "mu.upper"),label="counts")
p<-p %>% dyLegend(show = "always", hideOnMouseOut = FALSE) %>%  dyShading(from = days.ahead[1], to = days.ahead[step.ahead], color = "#CCEBD6")%>% dyEvent(days.ahead[1], "Prediction", labelLoc = "bottom")
p
```
...
upper<-apply(imp$samples[,2,],1,quantile,prob=p.upper)
slope<-xts(cbind(lower,med,upper), order.by = c(days,days.ahead), frequency = 7)
p <- dygraph(slope,main=paste(mycounty,": new case slope (confidence ",100*confidence,"%)",sep = ""),xlab="Day",height=400,width=800) %>% dySeries(c("lower", "med", "upper"),label="median")%>% dyLegend(show = "always", hideOnMouseOut = FALSE)
p<- p %>% dyShading(from = days.ahead[1], to = days.ahead[step.ahead], color = "#CCEBD6")%>% dyEvent(days.ahead[1], "Prediction", labelLoc = "bottom")
p
```