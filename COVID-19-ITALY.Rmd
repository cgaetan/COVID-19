...
* We want to investigate t the evolution of the coronavirus pandemic in Italy from a statistical perspective using aggregated data.

* Our point of view is that of surveillance with the goal of detecting important change in the underlying (random) process as soon as possible after it has occured.
...
In order to catch the evolution of the viral infection we employ a Dynamic Generalized Linear Model for the Poisson distribution with log-linear link function for the expected counts, $\mu_t$, namely
\begin{eqnarray*}
Y_t|\mu_t&\sim& \mbox{Poisson}(\mu_t),\qquad t=1,2,\ldots \\
\log(\mu_t)&=&\alpha_t\\
...
```{r}
model_poisson <- SSModel(counts ~ -1 +
                SSMtrend(2,Q=list(matrix(NA,1,1),matrix(NA,1,1))),distribution = "poisson")
